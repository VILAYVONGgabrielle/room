    ...
    for ($i = 0; $i < count($_SESSION['panier']['id_produit']); $i++) {
        ...
        for ($i = 0; $i < count($_SESSION['panier']['id_produit']); $i++){ 
            $id_produit = $_SESSION['panier']['id_produit'][$i];
                ...
                execRequete("INSERT INTO commandes VALUES (NULL, :id_membre, :id_produit,NOW())",array('id_membre'=> $id_membre, 'id_produit' => $id_produit));
...
?>
    ...
    <table class="table table-bordered table-striped">
        ...
        <tr>
            <?php
            for ($i = 0; $i < count($_SESSION['panier']['id_produit']); $i++) {
                $resultat = execRequete(" SELECT*FROM produits pdt
                INNER JOIN salles sal ON pdt.id_salle = sal.id_salle
                WHERE id_produit=:id_produit", 
                array('id_produit' => $_SESSION['panier']['id_produit'][$i]));
                $produits = $resultat->fetch();
               // $message = '';
              //while($produits = $resultat->fetch()){}
            ?>
                <td><?php echo $produits['id_produit'] ?></td>
                <td><?php echo $produits['id_salle'].' - '.$produits['titre'] ?></td>
                <td><?php echo $produits['date_arrivee'] ?></td>
                <td><?php echo $produits['date_depart'] ?></td>
                <td><?php echo number_format($produits['prix'], 2, ',', '&nbsp;') ?> &euro;</td>

                <td><a href="?action=supligne&id_produit=<?php echo $produits['id_produit'] ?>"><i class="fas fa-trash"></i></a></td>
        </tr>
    <?php
